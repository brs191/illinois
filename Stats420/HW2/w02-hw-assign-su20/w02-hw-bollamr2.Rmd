...
``` {r }
plot(Hwt ~ Bwt, data = cats, xlab = "Cats body weight in kg.", ylab = "Cats heart weight in g", pch = 20, cex = 1, col = "orange")
...
``` {r }
plot(response ~ predictor, data = sim_data, xlab = "Simulated Predictor Varaibe", ylab = "Simulated Response variable", 
...
legend("topright", c("Estimate", "Truth"), lty = c(1,2), lwd = 2, col = c("black", "pink"))
```
...
for (i in 1:1500) {
  x = runif(n = 25, 0, 10)
  sim_data = sim_slr(beta_0, beta_1, sigma)
...
``` {r }
hist(beta_hat_1, breaks = 50, main = "Histogram of beta_hat_1", col = "green")
```
***
...
```{r}
hist(beta_hat_1, breaks = 50, main = "Histogram of beta_hat_1", col = "dodgerblue")
```
...
library(knitr)
kable(ozone_summary, format.args = list(decimal.mark = "."))
