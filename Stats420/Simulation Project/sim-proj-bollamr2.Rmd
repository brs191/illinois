...
study2_results = data.frame(study2_models, avg_rmse_train[1, ], avg_rmse_tst[1, ], avg_rmse_train[2, ], avg_rmse_tst[2, ], avg_rmse_train[3, ], avg_rmse_tst[3, ])
...
calc_power = function(sig = 1, n = 10) {
  ...
  for (i in 1: length(beta1)) {
    p_value = rep(0, num_sims) #1000 reps
...
plot(power ~ beta, data = data.frame(power = power_1_10, beta = beta1), xlab = "beta values", ylab = "power", pch = 20, cex = 2)


# library(ggplot2)
# ggplot(data = data.frame(power = power_1_10, beta = beta1), aes(x = beta, y = power)) +
#   stat_smooth(method = "lm", se = FALSE, color = "green", fomulate = power ~ beta)  
```
