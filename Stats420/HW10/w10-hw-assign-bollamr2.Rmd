...
```
From the histogram plots, we notice that the curve doesn't follow this histogram distribution in both Wald Test and Likelihood Rate Test and the estimated and true probability will vary significantly.
...
Test statistic of the test is 
We can notice that p-value is less than the alpha (0.01) with a value of 2.42e-09. 
...
We can notice that p-value is less than the alpha (0.01) at 0.005399. 
We reject the NULL Hypothesis
Test statistic of the test is 
We prefer the model with **"pclass + sex + age + sex:age"** predictors (larger model), since the p-value is smaller than alpha.
...
Misclassification rate is at 0.2212.

Sensivity is at 0.6444

Specificity of this classifer is at 0.877
