...
msleep
```
There are 83 Observations and 11 Variables in Mammals Dataset.
...
birthwt
```
There are 189 Observations and 10 Variables in "birthwt" dataset.
...
#names(birthwt)
#?birthwt
plot(bwt ~ lwt, data = birthwt, xlab = "Mother's weight in pounds", ylab = "Birth weight of child in grams", pch = 20, cex = 1, col = "orange")
```
...
```{r }
plot(bwt ~ age, data = birthwt, xlab = "Mother's age in years", ylab = "Birth weight in grams", pch = 20, cex = 1, col = "blue")
```
We see a decreasing trend in the Birth weight with an increase in mother's age. But a increased birth weight at the age of 45 in the dataset looks unusual.
...
boxplot(bwt ~ smoke, data = birthwt, 
        ...
        xlab = "Smoke status during pregnancy (0 = non-smoker, 1 = smoker)", main = "birth weight vs smoke status",
        col = "green", border = "blue")
```

Average weight of babies born to smokers looks is less than the averge weight of the babies born to non-smokers. At the same time we need to note that there is high variation in the values.
...
hist(calories, xlab = "Calories in KCal", ylab = "Frequency")
```

Values around 400 and after 800 seems unusual. The distirbution of Calories in right-schewed.
...
``` {r }
plot(Calories ~ Protein, data = nut_data, main = "Calories vs Protein", xlab = "Protein in grams", ylab = "Calories in Kcal", pch = 20, cex = 0.1, col = "red")
```

As proteins value increases there seems to be a decrease in Calories count. That looks like a trend.
Knowing only protein content of a food could help in predicting the calories in the food. But maynot be a good predictor.
...
``` {r }
plot(Calories ~ I(4*Protein + 4*Carbs + 9*Fat), data = nut_data, xlab = "4*Protein + 4*Carbs + 9*Fat", ylab = "Calories in Kcal", pch = 20, cex = 0.1, col = "green")
```

Result doesn't look like a straight line. There could be many reasons for not being a straight line.
Food is made up of many components other than Proteins/Carbs/Fat. There could be a measurement error.
